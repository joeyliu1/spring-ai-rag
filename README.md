...
- 提供 RAG 检索质量评估接口，可评估 topK、命中关键词、平均相似度和召回片段，支持批量评测集和多策略对比。
...
| RAG 评估 | 检索命中、关键词覆盖率、相似度、召回片段明细、批量评测、多策略对比 |
...
也可以使用仓库内 `docker-compose.yml` 一键启动基础设施：
...
| `POST /api/v1/rag/evaluate/batch` | RAG 批量评测与策略对比 |
...
```text
docs/rag-evaluation-sample.json
```
...
- 实现批量 RAG 评测接口，支持基于 TopK、相似度阈值的检索效果对比，输出通过率、Recall@K 和 MRR。
